...
data "cloudinit_config" "user_data" {
  ...
  part {
    ...
    content      = file("${path.module}/scripts/frontend-dockerfile/DockerFile")
  }

  part {
    content_type = "text/x-shellscript"
    filename     = "userdata_frontend_dockerfile_script"
    content      = file("${path.module}/scripts/dockerScriptFrontend.sh")
  }

  part {
    content_type = "text/x-shellscript"
    filename     = "userdata_backend_dockerfile"
    content      = file("${path.module}/scripts/backend-dockerfile/DockerFile")
  }

  part {
    content_type = "text/x-shellscript"
    filename     = "userdata_backend_dockerfile_script"
    content      = file("${path.module}/scripts/dockerScriptBackend.sh")
  }
